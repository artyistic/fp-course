...
findM mpred (t :. ts) =
  (\b -> if b then pure $ Full t else findM mpred ts) =<< mpred t
...
isHappy ::
  Integer ->
  Bool
isHappy = contains 1 . firstRepeat . happySeq . fromInteger
  where square = join (*)
        numToDigits x = map (square . C.digitToInt) (show' x)
        happySeq = produce (sum . numToDigits)