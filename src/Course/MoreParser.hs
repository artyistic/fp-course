...
between ::
  ...
  -> Parser a
  -> Parser a
between = flip . lift3 (const const)
...
hex ::
  Parser Char
hex =  optional chr '\NUL' . readHex <$> replicateA 4 (satisfy isHexDigit)
-- readHex really wont fail for values from 0 to 16^4 - 1
...
betweenSepbyComma ::
  ...
  -> Parser a
  -> Parser (List a)
betweenSepbyComma x y pa = betweenCharTok x y $ sepby pa (charTok ',')
