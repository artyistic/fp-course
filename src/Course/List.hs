...
flattenAgain ::
  List (List a)
  -> List a
-- flattenAgain = flatMap _f, type hole is (List a -> List a)
...
seqOptional ::
  List (Optional a)
  -> Optional (List a)
-- seqOptional = error "FOO"
seqOptional Nil = Full Nil
seqOptional (t :. ts) = case t of
  Full a -> bindOptional (\x -> Full (a :. x)) (seqOptional ts)
  Empty -> Empty
